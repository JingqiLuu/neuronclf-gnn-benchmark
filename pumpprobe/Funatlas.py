...
class Funatlas:
    ...
    module_folder = os.path.dirname(os.path.abspath(pp.__file__)) + os.sep
    print({module_folder})
    ...
    def from_datasets(
            ...
            merge_AWC=None,
            signal="green.txt",signal_kwargs={},load_signal=True,verbose=True,
            ...
            if load_signal:
                if signal == "ratio":
                    sig = wormdm.signal.Signal.from_signal_and_reference(
                                           folder,"green.txt","red.txt",**signal_kwargs)
                else:
    ...
    def get_qvalues(self, occ1, occ3, exclude_zero_k):
        ...
        if calc_everything:   
            for f in range(len(folders)):
                ...
                # Load the signal
                sig = wormdm.signal.Signal.from_file(folders[f], "green.txt")
                sig.appl_photobl()
                sig.remove_spikes()
    ...
    def get_ctrl_distributions(cls,strain=""):
        ...
        if load_cached_distributions:
            ...
            for i_ds in np.arange(n_ds):
                ...
                sig = wormdm.signal.Signal.from_file(folder,"green.txt",**signal_kwargs)
    ...
    def load_innexin_expression_from_file(self):
        if not self.merge_bilateral:
            ...
            f = open(self.module_folder+self.fname_innexins,"r",encoding='utf-8-sig')
            lines = f.readlines()
            f.close()
